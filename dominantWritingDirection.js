...
function dominantDirection( text ){
  ...
  for ( var i = 0; i < text.length; i++ ){
    if (characterScript( text.codePointAt( i ) ) == "ltr")
    	l.push("ltr");

    else if (characterScript( text.codePointAt( i ) ) == "rtl")
    	r.push("rtl");

    else if (characterScript( text.codePointAt( i ) ) == "ttb")
    	t.push("ttb")

    else
      continue;
  }
  	return l.length > r.length ? "ltr" : (r.length < t.length ? "ttb" : "rtl");
}

console.log(dominantDirection("Heyooooooo"));
